...
| `professionals` | `professional` | Profil public + métriques + statut abonnement |
...
| `subscriptions` | Système → `professional` | Ledger immuable des abonnements et paiements |
...
### Colonnes Abonnement (modifiables partiellement)

| Colonne | Type | Notes |
|---|---|---|
| `subscription_plan` | TEXT | `free` / `premium` |
| `subscription_status` | TEXT | `active` / `expired` / `canceled` / `past_due` |
| `subscription_expires_at` | TIMESTAMPTZ | Date de fin de validité |
...
| Colonne | Type | Notes |
|---|---|---|
| `is_visible` | BOOLEAN | **Colonne générée** — `TRUE` si `subscription_status = 'active' AND is_active`. Ne jamais écrire |
| `is_active` | BOOLEAN | Mis à `FALSE` si non-respect grave de la charte |
...
| UPDATE paramètres Abonnement | Pro (son propre profil) | `subscription_plan` |
| ALL | `admin` | |
...
Ledger financier immuable. Chaque transaction d'abonnement est enregistrée ici.
...
| `status` | TEXT | Statut au moment de la transaction |
| `amount` | NUMERIC(10,2) | Montant payé |
| `currency` | TEXT | `EUR` ou `XOF` |
| `payment_method` | TEXT | `stripe` / `wave` / `orange_money` |
| `payment_id` | TEXT | Référence paiement externe |
| `starts_at` | TIMESTAMPTZ | Début de période |
| `expires_at` | TIMESTAMPTZ | Fin de période |
| `created_at` | TIMESTAMPTZ | |
...
| Table | Colonnes |
|---|---|
| `professionals` | `status`, `recommendation_count`, `signal_count`, `avg_rating`, `positive_review_pct`, `review_count`, `subscription_plan`, `subscription_status`, `subscription_expires_at`, `total_views`, `current_month_views`, `is_visible`, `is_active` |
...
| `subscriptions` | Toute la table (INSERT système uniquement) |
