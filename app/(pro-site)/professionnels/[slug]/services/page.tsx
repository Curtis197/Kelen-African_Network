...
export default async function ServicesListPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <ProSiteStyleProvider cornerStyle={settings?.cornerStyle ?? 'rounded'} colorMode={settings?.colorMode ?? 'light'} logoColor={settings?.logoColor ?? null} imageWeight={settings?.imageWeight} spacing={settings?.spacing}>
      ...
      <main>
        <ProSiteListPage slug={pro.slug} sectionPath="services" sectionTitle="Services" proName={proName} profession={pro.category ?? ''} items={items} />
      </main>
