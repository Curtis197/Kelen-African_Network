// app/(pro-preview)/pro/preview/[slug]/produits/page.tsx
...
export default async function PreviewProduitsPage({ params, searchParams }: Props) {
  ...
  return (
    <ProSiteStyleProvider cornerStyle={cornerStyle} colorMode={colorMode} logoColor={settings?.logoColor ?? null} imageWeight={imageWeight} spacing={spacing}>
      ...
      <ProSiteNav slug={slug} proName={proName} showServices={settings?.showServices ?? true} showRealisations={settings?.showRealisations ?? true} showProduits calendarUrl={null} basePath={basePath} />
      <main>
        <ProSiteListPage slug={slug} sectionPath="produits" sectionTitle="Produits" proName={proName} profession={pro.category ?? ''} items={items} basePath={basePath} />
      </main>
