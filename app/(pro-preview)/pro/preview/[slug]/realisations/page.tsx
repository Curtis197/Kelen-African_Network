// app/(pro-preview)/pro/preview/[slug]/realisations/page.tsx
...
export default async function PreviewRealisationsPage({ params, searchParams }: Props) {
  ...
  return (
    <ProSiteStyleProvider cornerStyle={cornerStyle} colorMode={colorMode} logoColor={settings?.logoColor ?? null} imageWeight={imageWeight} spacing={spacing}>
      ...
      <ProSiteNav slug={slug} proName={proName} showServices={settings?.showServices ?? true} showRealisations showProduits={settings?.showProduits ?? true} calendarUrl={null} basePath={basePath} />
      <main>
        <ProSiteListPage slug={slug} sectionPath="realisations" sectionTitle="Réalisations" proName={proName} profession={pro.category ?? ''} items={items} basePath={basePath} />
      </main>
