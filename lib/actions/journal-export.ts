...
export async function generateJournalPDFData(
  ...
  return {
    ...
    logs: exportLogs.map(log => ({
      ...
      money_currency: log.money_currency,
      ...
      media: (log.media || []).map(m => ({
        ...
        exif_timestamp: m.exif_timestamp || undefined,
        exif_latitude: m.exif_latitude || undefined,
        exif_longitude: m.exif_longitude || undefined,
        signedUrl: m.signedUrl || undefined,
      })),
