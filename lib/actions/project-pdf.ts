...
export async function generateProjectPdf(projectId: string): Promise<{ success: boolean; html?: string; error?: string }> {
  console.log('[PDF-EXPORT] Generating PDF for project:', projectId);
  ...
  if (projectError || !project) {
    console.error('[PDF-EXPORT] Project not found:', projectError?.message);
    return { success: false, error: "Projet non trouvé" };
  }

  console.log('[PDF-EXPORT] Project fetched:', project.title);
  ...
  const formatCurrency = (amount: number, currency: string) => {
    const symbols: Record<string, string> = { EUR: '€', XOF: 'CFA', USD: '$' };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'in_progress': return '<span class="timeline-status status-progress">⟳ En cours</span>';
      case 'pending': return '<span class="timeline-status status-pending">⏳ À venir</span>';
...
<body>
  ...
  console.log('[PDF-EXPORT] HTML generated successfully, length:', html.length);
