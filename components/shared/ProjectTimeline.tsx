        ...
        <div 
          ...
          className={`p-8 rounded-[2.5rem] border transition-all duration-500 hover:shadow-2xl ${
            phase.status === 'IN PROGRESS' 
              ? 'bg-white shadow-xl shadow-stone-200/50 border-kelen-green-500/30 ring-4 ring-kelen-green-500/5 relative overflow-hidden' 
              : phase.status === 'COMPLETED'
              ? 'bg-white shadow-sm border-stone-100'
              : 'bg-stone-50/50 border-stone-100 opacity-60 grayscale hover:grayscale-0 hover:opacity-100 hover:bg-white'
          }`}
        >
          {phase.status === 'IN PROGRESS' && (
          ...
          <div className="flex items-start justify-between mb-8 relative z-10">
            <div className="flex items-center gap-5">
              ...
              <div>
                <h4 className={`text-2xl font-black font-headline leading-tight ${
                  phase.status === 'UPCOMING' ? 'text-stone-400' : 'text-stone-900'
                }`}>
                  {phase.title}
                </h4>
                <p className={`text-sm font-medium italic ${
                  phase.status === 'UPCOMING' ? 'text-stone-300' : 'text-stone-500'
                }`}>
                  {phase.description}
                </p>
              </div>
            ...
            <div className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border ${
              phase.status === 'COMPLETED'
                ? 'bg-kelen-green-50 text-kelen-green-700 border-kelen-green-100'
                : phase.status === 'IN PROGRESS'
                ? 'bg-white text-stone-900 border-stone-200 shadow-sm flex items-center gap-2'
                : 'bg-stone-100 text-stone-400 border-stone-200'
            }`}>
              {phase.status === 'IN PROGRESS' && <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span>}
          ...
          <div className="space-y-4 relative z-10">
             ...
             {(phase.last_update || phase.next_up) && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8">
                   {phase.last_update && (
                      <div className="bg-stone-50 p-5 rounded-2xl border border-stone-100 group-hover:bg-stone-100 transition-colors">
                         <p className="text-[9px] font-black text-stone-400 uppercase tracking-[0.2em] mb-1.5">Récemment</p>
                         <p className="text-sm font-bold text-stone-900">{phase.last_update}</p>
                      </div>
                   )}
                   {phase.next_up && (
                      <div className="bg-stone-900 p-5 rounded-2xl shadow-xl transition-transform hover:-translate-y-1">
                         <p className="text-[9px] font-black text-kelen-green-400/60 uppercase tracking-[0.2em] mb-1.5">Prochaine Étape</p>
                         <p className="text-sm font-bold text-white">{phase.next_up}</p>
                      </div>
                   )}
                </div>
             )}
