...
export function AddProSearchDialog({
  ...
  const handleSearch = useCallback(
    debounce(async (query: string) => {
      ...
      if (error) {
        console.error('[ADD_PRO_SEARCH] Error:', error.message);
        toast.error("Erreur lors de la recherche");
      } else {
        console.log('[ADD_PRO_SEARCH] Results found:', data?.length);
        setResults(data || []);
      }
      setIsSearching(false);
    }, 500),
  ...
  const handleAddPro = async (pro: Professional) => {
    setIsAdding(pro.id);
    console.log('[ADD_PRO_SEARCH] Adding pro to project:', { projectId, proId: pro.id, area: areaName });
    ...
    try {
      ...
      console.error('[ADD_PRO_SEARCH] Runtime error:', error);
