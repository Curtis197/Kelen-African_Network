...
Kelen is a permanent accountability platform designed to foster trust in collaborations between the African client and professionals in Africa. It operates on two distinct, independent systems:
...
*   **Authentication:** Secure registration and login flows for both "user" (client) and "professional" roles, handled via Supabase Auth.
...
*   **client User Area (Authenticated):**
...
### Phase 2: User Authentication & client Workflow

**Objective:** To enable client users to register, log in, and contribute to the platform by submitting evidence.
...
3.  **client Dashboard (`/dashboard`):** Build the user's personal dashboard to list their past and pending submissions.
